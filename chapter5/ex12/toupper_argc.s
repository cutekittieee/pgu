...
#            If zero parameters are given, STDIN and STDOUT are used.
#
#PROCESSING: 1) Open the input file (might be STDIN, which does not need to be opened)
#            2) Open the output file (might be STDOUT, which does not need to be opened)
...
.equ NUMBER_ARGUMENTS, 3 #we expect 3 arguments, including the name of the program (argc = 3)
                         #argc = 1, if 0 arguments are given on the command line
...
#number of parameters is stored in esi
movl  ST_ARGC(%ebp), %esi

open_files:
open_fd_in:
#if the argument count is not 3, STDIN and STDOUT are used
cmpl $3, %esi
jne store_fd_stdin
###OPEN INPUT FILE###
#open syscall
...
end_loop:
#if the argument count is not 3, closing is not necessary (STDIN and STDOUT)
cmpl $3, %esi
jne end_program
###CLOSE THE FILES###
