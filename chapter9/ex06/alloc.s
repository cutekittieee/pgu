...
#1 if the memory manager has been initialized. Otherwise, its value is 0.
initialized:
.byte 0

#Text written to STDOUT when malloc is called
malloc_str:
.ascii "malloc has been called.\n\0"

#Text written to STDOUT when free is called
free_str:
.ascii "free has been called.\n\0"
...
.equ SYS_WRITE, 4   #system call number for the write
                    #system call
...
cmpb  $0, initialized
jne   cont_alloc              #if not 0, it has already been initialized
call  allocate_init           #if 0, we call the initialize function
movb  $1, initialized         #here we set the flag to 1
