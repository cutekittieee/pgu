...
# We try to split the memory region
subl  %ecx, %edx
subl  $HEADER_SIZE, %edx
# If 0 or less bytes left for the new region, we cannot create a new region
# so we just allocate it and return
cmpl  $0, %edx
jle   alloc_ret

# We calculate the end of the region using the requested size
# %ebx will point to the beginning of the new memory region
movl  %eax, %ebx
addl  $HEADER_SIZE, %ebx
addl  %ecx, %ebx

# We set up a new header at the start of the new memory region
movl  $AVAILABLE, HDR_AVAIL_OFFSET(%ebx)
movl  %edx, HDR_SIZE_OFFSET(%ebx)

# We set the size of the original memory region
